...
func GetClientCommand(ip string, ports string, language string, excludeports string, timeout int) string {
    ...
    languageList["c"] = `echo '#include <stdio.h>\r\n#include <string.h>\r\n#include <stdlib.h>\r\n#include <arpa/inet.h>\r\n#include <unistd.h>\r\nint main(int argc , char **argv){int err, i , sock, list[]= {` + listPort + `} ;char hostname[100] = "` + ip + `";struct sockaddr_in sa;strncpy((char*)&sa , "" , sizeof sa);sa.sin_family = AF_INET;for( i = 0 ; i <= sizeof(list); i++) {sa.sin_port = htons(list[i]);sock = socket(AF_INET , SOCK_STREAM , 0);if(sock < 0){exit(1);}err = connect(sock , (struct sockaddr*)&sa , sizeof sa);if( err < 0 ){fflush(stdout);}else{printf("%-5d open\\n",  list[i]);}close(sock);}fflush(stdout);return(0);}' > a.c && gcc a.c -o a && ./a`
